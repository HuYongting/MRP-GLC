...
parser.add_argument('--version', type=str, default="Two Branch and Matrix fusion")
parser.add_argument('--lr', type=float, default=2e-4, help='initial learning rate')
parser.add_argument('--para', type=str, default="memory=10,loss=int+0.1*grad")
...
model_str = 'batch='  + str(args.batch_size) + '_lr='+ str(args.lr) + "_" + args.para
model_path = os.path.join(root_path, model_str)
makedir(model_path)
logger = get_logger(os.path.join(model_path, time_ + "_exp.log"))
...
start_epoch = -1
if args.RESUME:
    path_checkpoint = "/home/zlab/disk1/hu/ThreeUnet/results/avenue/Two Branch and Matrix fusion/" \
                      "checkpoint.pth"
...
for epoch in range(start_epoch + 1, args.epochs):
    model.train()
    for j, (datas) in enumerate(train_batch):
        imgs = datas.cuda() # 4,5,3,256,256
        pred_img, sim = model.forward(imgs[:, :12, :])
        loss_int = intensity_loss(pred_img, imgs[:, 12:, :])
        loss_grad = gradient_loss(pred_img, imgs[:, 12:, :])
        loss = loss_int + 0.1 * loss_grad
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        if ((j + 1) % args.print_iter == 0):  # shanghai:10000    avenue: 1000   ped2: 200
            logger.info('[{:04d} / {:04d}]   loss:{:.8f}   int:{:.8f}    grad:{:.8f}'.
                        format(epoch + 1, j + 1, loss.item(),loss_int.item(),loss_grad.item()))
    ...
    if ((epoch + 1) % args.save_epochs == 0):
        ...
        torch.save(checkpoint, os.path.join(model_path, 'checkpoint_{}.pth'.format(epoch + 1)))
