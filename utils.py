...
''' this is the implementation of the multi anomaly score'''
...
#     '''
#     patch : the num of the patch most big values
#     size :  win size
#     step :  win step
#     is_multi : the sum of the patch big values
#     '''
#     assert size % step == 0
...
loss_func_mse = nn.MSELoss(reduction='none')
def batch_mse(imgs,outputs):
    mse = []
    for i in range(imgs.shape[0]):
        m = torch.mean(loss_func_mse((outputs[i, :] + 1) / 2, (imgs[i, 4 * 3:, :, :] + 1) / 2), dim=[1, 2, 3])
        mse.append(m)
    return m

''' 这里计算的是一个batch,用卷积实现的求取最大的mse'''
def max_mse(mse_img,kernel_size,step):
    b,c,h,w = mse_img.size()
    conv = torch.nn.Conv2d(c, c, (kernel_size, kernel_size), stride=(step,step), bias=False)
    conv.weight.data = torch.ones(c,c,kernel_size,kernel_size)
    mse_list = torch.mean(conv(mse_img),dim=1)
    mse_list = max(mse_list.flatten().tolist())
    return mse_list

# def multi_patch_max_mse(diff_map_appe):
#     mse_16 = max_mse(diff_map_appe, kernel_size=16, step=4)
#     mse_32 = max_mse(diff_map_appe, kernel_size=32, step=8)
#     mse_64 = max_mse(diff_map_appe,kernel_size=64, step=16)
#     mse_128 =max_mse(diff_map_appe, kernel_size=128, step=32)
#     return mse_16,mse_32,mse_64,mse_128
...
def multi_patch_max_mse(diff_map_appe):
    # mse_16 = patch_max_mse(diff_map_appe, patches=3, size=16, step=8, is_multi=False)
